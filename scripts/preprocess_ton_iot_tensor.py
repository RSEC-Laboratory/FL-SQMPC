#!/usr/bin/env python3
"""Preprocess ToN-IoT CSV into a tensor-split directory compatible with
...
All ToN-IoT features are numeric (counts, byte stats, encoded categoricals),
so we treat every feature as continuous and emit an empty categorical schema.
The "log_scale" preprocessing flag is honored to match the FL pipeline's
behavior on heavy-tailed columns.
"""
...
DEFAULT_CSV = ROOT / "ToN_IoT_processed" / "ToN_IoT_extracted.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=ROOT / "ton_iot_processed_tensor")
    ...
    parser.add_argument("--log-scale", action="store_true", default=True,
                        help="Apply log1p before MinMax scaling (matches FL pipeline default for ToN-IoT).")
...
def main() -> None:
    ...
    if args.task == "binary":
        ...
        label_map = {name: idx for idx, name in enumerate(observed)}
    ...
    meta = {
        ...
        "dataset_id": "ton_iot_tensor" if args.task == "multiclass" else "ton_iot_tensor_binary",
        "display_name": "ToN-IoT (tensor split)",
        "dropped_columns": [],
        "label_map": label_map,
